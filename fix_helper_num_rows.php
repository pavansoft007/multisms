...
echo "<h1>Fix Mobile Cards Helper (num_rows issue)</h1>";
...
// Replace the num_rows() method with a check for the object type
$new_content = str_replace(
    'if ($query->num_rows() > 0) {',
    'if ((is_object($query) && method_exists($query, "num_rows") && $query->num_rows() > 0) || (is_array($query) && count($query) > 0)) {',
    $original_content
);

// Replace other occurrences of num_rows()
$new_content = str_replace(
    '$has_config = ($check_query->num_rows() > 0);',
    '$has_config = (is_object($check_query) && method_exists($check_query, "num_rows")) ? ($check_query->num_rows() > 0) : (is_array($check_query) && count($check_query) > 0);',
    $new_content
);
...
echo "<p>The mobile_cards_helper.php file has been updated to handle different object types.</p>";
...
$restore_file = 'restore_helper_num_rows.php';
